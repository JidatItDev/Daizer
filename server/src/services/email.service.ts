...
export class EmailService {
  ...
  private static async getTemplate(type: string) {
    ...
    if (!tpl) throw new Error(`Email template not found for type: ${type}`);
    return tpl;
  }

  // replace {{placeholders}} with actual values
  private static applyPlaceholders(
    body: string,
  ...
  static async sendTemplateEmail(
    ...
    const tpl = await this.getTemplate(type);
    const subject = this.applyPlaceholders(tpl.subject, variables);
    const body = this.applyPlaceholders(tpl.body, variables);

    const html = this.getBaseTemplate(subject, body);
